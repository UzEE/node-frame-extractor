...
module.exports = function frameExtractor(options) {
	...
	self.init = function (opts) {
		...
		if (opts.pushToCloud) {
			...
			self.config.awsRegion = opts.awsRegion;
			self.config.awsProfile = opts.awsProfile;
			self.config.pushToCloud = opts.pushToCloud;
		}

		self.config.outDirPrexix = opts.outDirPrexix || 'frames';

		self.config.outDir = opts.outDir || path.join(path.dirname(path.resolve(self.config.video || process.cwd())), self.config.outDirPrexix + "." + self.config.videoId);
	...
	self.pushToCloud = function(cb) {

		if (!self.files || !self.files.length) {
			return cb(new Error('No files to push'));
		}

		var credentials = new AWS.SharedIniFileCredentials({profile: self.config.awsProfile});
		...
		async.eachLimit(
			...
			function pushFileToCloud(file, callback) {
				...
				var params = {
					...
					Key: "video." + self.config.videoId + "/" + path.basename(file),
					Body: fileStream,
					ACL: "public-read",
			...
			function pushFileToCloudComplete(err) {
